...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the FeltDB component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://feltdb.com/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
...
async function applyComponentTagger(appPath: string) {
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[feltdb] add FeltDB component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[feltdb] add Capacitor for mobile app support",
    });
