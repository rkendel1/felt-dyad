...
export async function getModelClient(
  ...
  // Handle the managed AI override
  if (dyadApiKey) {
    // Check if the selected provider supports managed AI (has a gateway prefix) OR
    ...
    if (providerConfig.gatewayPrefix != null || dyadEngineUrl) {
      ...
      logger.info(
        `\x1b[1;97;44m Using managed AI API key for model: ${model.name} \x1b[0m`,
      );

      logger.info(
        `\x1b[1;30;42m Using managed AI engine: ${dyadEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      logger.warn(
        `Managed AI is enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
