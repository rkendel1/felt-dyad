...
export async function getModelClient(
  ...
  // Handle FeltDB AI override
  if (dyadApiKey && settings.enableDyadPro) {
    // Check if the selected provider supports FeltDB AI (has a gateway prefix) OR
    ...
    if (providerConfig.gatewayPrefix != null || dyadEngineUrl) {
      ...
      logger.info(
        `\x1b[1;97;44m Using FeltDB AI API key for model: ${model.name} \x1b[0m`,
      );

      logger.info(
        `\x1b[1;30;42m Using FeltDB AI engine: ${dyadEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      logger.warn(
        `FeltDB AI enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
